...
class Ismuba_model extends CI_Model
{
    public function get_ismuba_profile()
    {
        return $this->db->get_where('ismuba', ['is_active' => 1])->row_array();
    }

    public function get_ismuba_struktur()
    {
        return $this->db->get_where('ismuba_struktur', ['is_active' => 1])->result_array();
    }

    public function isi_profile()
    {
        date_default_timezone_set('Asia/Jakarta');
        $nama_ismuba = htmlspecialchars($this->input->post('nama_ismuba'));
        $deskripsi_ismuba = htmlspecialchars($this->input->post('deskripsi_ismuba'));
        ...
        if ($program_kerja) {
            $config['upload_path']  = './assets/file_upload/ismuba/profile/';
            $file_name_underscore = str_replace(' ', '_', $nama_ismuba);
            ...
            redirect('ismuba_be/index');
        }

        $this->db->set('nama_ismuba', $nama_ismuba);
        $this->db->set('deskripsi_ismuba', $deskripsi_ismuba);
        $this->db->set('program_kerja', $program_kerja_new);
        $this->db->insert('ismuba');
    }
    ...
    public function tambah_struktur()
    {
        ...
        if ($foto) {
            $config['upload_path']  = './assets/file_upload/ismuba/struktur/';
            ...
            if ($this->upload->do_upload('foto')) {
                ...
                if (!$this->image_lib->resize()) {
                    ...
                    $config['source_image'] = './assets/file_upload/ismuba/struktur/' . $foto_source['file_name'];
                ...
                $config['quality'] = '85%';
                $config['new_image'] = './assets/file_upload/ismuba/struktur/' . $foto_source['file_name'];
        ...
        $this->db->insert('ismuba_struktur');
    }
    ...
    public function ubah_struktur()
    {
        ...
        $this->db->set('divisi', $divisi);
        $this->db->where('id', $id);
        $this->db->update('ismuba_struktur');
    }
