...
{
    public function get_humas_profile()
    {
        return $this->db->get_where('humas', ['is_active' => 1])->row_array();
    }

    public function get_humas_struktur()
    {
        return $this->db->get_where('humas_struktur', ['is_active' => 1])->result_array();
    }

    public function isi_profile()
    {
        date_default_timezone_set('Asia/Jakarta');
        $nama_humas = htmlspecialchars($this->input->post('nama_humas'));
        $deskripsi_humas = htmlspecialchars($this->input->post('deskripsi_humas'));
        ...
        if ($program_kerja) {
            $config['upload_path']  = './assets/file_upload/humas/profile/';
            $file_name_underscore = str_replace(' ', '_', $nama_humas);
            ...
            redirect('humas_be/index');
        }

        $this->db->set('nama_humas', $nama_humas);
        $this->db->set('deskripsi_humas', $deskripsi_humas);
        $this->db->set('program_kerja', $program_kerja_new);
        $this->db->insert('humas');
    }
    ...
    public function tambah_struktur()
    {
        ...
        if ($foto) {
            $config['upload_path']  = './assets/file_upload/humas/struktur/';
            ...
            if ($this->upload->do_upload('foto')) {
                ...
                if (!$this->image_lib->resize()) {
                    ...
                    $config['source_image'] = './assets/file_upload/humas/struktur/' . $foto_source['file_name'];
                ...
                $config['quality'] = '85%';
                $config['new_image'] = './assets/file_upload/humas/struktur/' . $foto_source['file_name'];
